...
openWeekMenu.addEventListener('click', async (event) => {
    ...
    try {
        weekContent.innerHTML = ''; // Clear previous content
...
searchButton.addEventListener('click', () => {
    ...
    `;
    // Populate the table with filtered restaurants
    filteredRestaurants.forEach(restaurant => {
        const tr = document.createElement('tr');
        
        // Add click event listener to focus on the restaurant's marker
        tr.addEventListener('click', function () {
            const [longitude, latitude] = restaurant.location.coordinates; // Extract coordinates
            if (map) {
                map.flyTo([latitude, longitude], 15, {animate: true, duration: 0.5}); // Center the map on the restaurant's location
                highlightMarker(restaurant._id); // Highlight the marker
            } else {
...
function createRestaurantCells(restaurant, tr) {
    ...
    tr.append(nameTd, cityTd); // removed addressTd
  }
  ...
  // hae tietyn ravintolan viikko menu
